...
class Tokenizer
{
  ...
  private:
    ...
    inline static bool IsSimpleIdentifierStart(unic_t ch) {
        return IsAsciiLetter(ch) || CharIn<'$', '_'>(ch);
    }
    static bool IsComplexIdentifierStart(unic_t ch);

    inline static bool IsSimpleIdentifierContinue(unic_t ch) {
        return IsAsciiLetter(ch) || IsDigit(ch) || CharIn<'$', '_'>(ch);
    }
    inline static bool IsComplexIdentifierContinue(unic_t ch);

    inline static bool IsHereStringNameStart(unic_t ch) {
        return CharInRange<'A', 'Z'>(ch);
    }

    inline static bool IsHereStringNameContinue(unic_t ch) {
        return CharInRange<'A', 'Z'>(ch) || CharInRange<'0', '9'>(ch);
    }
    ...
    inline static bool IsOctalDigit(unic_t ch) {
